...
_latitude*_ - широта места, float;

_longitude*_ - долгота места, float;

_radius*_ - радиус области покрытия в метрах, int;

_name*_ - название. String maxLength: 100, minLength: 5;
...
которых попадают координаты. Из списка пользователь может выбрать одно/несколько мест или вообще не выбирать => запрос на бэк: поиск мест, в область которых попадают координаты.
При запросе на добавление нового события в **NewEventDto** добавится лист id мест, которые выбрал пользователь. Та же самая логика при изменении координат во время обновления событий.
...
- добавление фильтров в эндпоинт GET /admin/events: 
...
Сможет просмотреть существующие места для проведения событий и список опубликованных событий, которые относятся к этому месту, а также искать события, задав область поиска.
- получение локаций GET /places;
- получение локации по id GET /places/{placeId};
- добавление фильтров в публичный эндпоинт GET /events:
  - область поиска lat, lon, radius - координаты центра и радиус поиска;
  - поиск событий в конкретных местах places - список идентификаторов мест.
...
_radius_ - радиус области покрытия локации;

_name_ - название локации;

_type_id_ - id типа локации;

_season(default: ALL)_ - в какой сезон доступно место;

_feature(default: null)_ - особенности: indoors, outdoors, around the clock, free.

`location_type` - возможные типы локации, заполняется предварительно.

`events_places` - вспомогательная таблица для связи событий и мест. _ManyToMany_: у события может быть указано несколько мест, в одном могут проводиться несколько событий.
